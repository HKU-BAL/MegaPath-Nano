...
def processTaxID(bam_path,taxon,threads,REFSEQ_PATH):
    ...
    try:
        acc_id_list = subprocess.check_output(
            "samtools view -@ {t} {bam}|cut -f3|sort|uniq".format(t=threads,bam=bam_path) ,encoding='utf-8',
...
def processAccessionNo(acc_id,taxon,threads):
    ...
    os.system("samtools view -@ {t} -b sample_{acc_id}.sam > sample_{acc_id}.bam".format(t=threads,acc_id=acc_id))
    ...
    os.system("bcftools mpileup -R sample_{acc_id}.merged.bed -Ou -f ref_{acc_id}.fa sample_{acc_id}.bam | bcftools call -Oz -mv --threads {t} -o calls_{acc_id}.vcf.gz".format(acc_id=acc_id,t=threads))
...
def main():
    ...
    parser.add_argument('inputbam',help='input bam')
    ...
    inputfile = results.inputbam
    outputfile = results.outputdir
    taxon = results.taxon
    threads=results.threads
    REFSEQ_PATH=results.REFSEQ_PATH
    ...
    if not os.path.exists("sample.sorted.bam") or not os.path.exists("sample.sorted.bam.bai") or not os.path.exists("header.sam") :
        p = subprocess.Popen("samtools sort {bam_path} -o sample.sorted.bam".format(bam_path), shell=True)
        p.communicate()
        ...
        s = subprocess.Popen("samtools view -@ {threads} -H sample.sorted.bam > header.sam".format(threads), shell=True)
