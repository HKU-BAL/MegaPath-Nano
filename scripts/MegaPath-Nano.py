...
from lib.aligner_reassignment_paf import Align
...
def step_adaptor_trimming(nanopath, adaptor_trimming):
    ...
    adaptor_trimming.O.query_filename_list['path'] = adaptor_trimming.O.query_filename_list['path'].map(lambda x: os.path.join(nanopath.temp_dir_name, os.path.split(x)[1] + '.adaptor_trimmed'))

    trimming_process_list = []

    for file_index, filename in enumerate(adaptor_trimming.I.query_filename_list['path']):
        trimming_command = ['qcat', nanopath.global_options['qcatThreadOption'], '--trim', '-f', filename, '-o', adaptor_trimming.O.query_filename_list['path'][file_index]]
...
def step_read_trimming_and_filter(nanopath, read_trimming_and_filter):

    nanopath.log.print('start')
    nanopath.log.print_time()
    nanopath.log.print('Read trimming:')
    nanopath.log.print('- head:  {crop} '.format(crop=read_trimming_and_filter.I.head_crop))
    nanopath.log.print('- tail:  {crop} '.format(crop=read_trimming_and_filter.I.tail_crop))
    nanopath.log.print('Filtering criteria:')
    nanopath.log.print('- read length >= {min_read_length} '.format(min_read_length=read_trimming_and_filter.I.min_read_length))
    nanopath.log.print('- average quality >= {min_quality} '.format(min_quality=read_trimming_and_filter.I.min_read_quality))

    RAM_dir_name = tempfile.mkdtemp(prefix='read_trimming_and_filter.', dir=nanopath.RAM_dir_name)

    file_prefix_with_path = os.path.join(nanopath.output_folder, nanopath.output_prefix)

    # Intermediate files between steps are placed in nanopath.temp_dir_name
    read_trimming_and_filter.O.query_filename_list = read_trimming_and_filter.I.query_filename_list.copy()
    read_trimming_and_filter.O.query_filename_list['path'] = read_trimming_and_filter.O.query_filename_list['path'].map(lambda x: os.path.join(nanopath.temp_dir_name, os.path.split(x)[1] + '.trimmed_and_filtered'))
    ...
    for file_index, filename in enumerate(read_trimming_and_filter.I.query_filename_list['path']):
        ...
        filter_command = [os.path.join(nanopath.global_options['tool_folder'], 'nanofastq'),]
    ...
    nanopath.log.print('Number of reads before filter: {num_read}'.format(num_read=read_trimming_and_filter.O.num_read))
    nanopath.log.print('Number of reads passed filter: {num_read}'.format(num_read=read_trimming_and_filter.O.num_read_passed_filter))
    nanopath.log.print('end')


def step_human_and_decoy_filter(nanopath, human_and_decoy_filter):

    nanopath.log.print('start')
    nanopath.log.print_time()
    nanopath.log.print('Filtering criteria for human:')
    nanopath.log.print('- alignment score >= {threshold} '.format(threshold=human_and_decoy_filter.I.human_min_alignment_score))
    nanopath.log.print('  or')
    nanopath.log.print('- alignment score / read length >= {threshold:.2%} '.format(threshold=human_and_decoy_filter.I.human_min_alignment_score_percent/100))
    nanopath.log.print('Filtering criteria for decoy:')
    nanopath.log.print('- alignment score >= {threshold} '.format(threshold=human_and_decoy_filter.I.decoy_min_alignment_score))
    nanopath.log.print('  or')
    nanopath.log.print('- alignment score / read length >= {threshold:.2%} '.format(threshold=human_and_decoy_filter.I.decoy_min_alignment_score_percent/100))

    temp_dir_name = tempfile.mkdtemp(prefix='human_and_decoy_filter.', dir=nanopath.temp_dir_name)
    RAM_dir_name = tempfile.mkdtemp(prefix='human_and_decoy_filter.', dir=nanopath.RAM_dir_name)
    ...
    align_list = Align(
                       assembly_metadata=nanopath.assembly_metadata,
                       ...
                       aligner_options=shlex.split(nanopath.global_options['alignerThreadOption'] + ' -x map-ont'),
    ...
    human_and_decoy_filter.O.query_filename_list['path'] = human_and_decoy_filter.O.query_filename_list['path'].map(lambda x: os.path.join(nanopath.temp_dir_name, os.path.split(x)[1] + '.human_and_decoy_filtered'))
    ...
    for file_index, filename in enumerate(human_and_decoy_filter.I.query_filename_list['path']):
        ...
        filter_command = [os.path.join(nanopath.global_options['tool_folder'], 'seqtk'),]
    ...
    nanopath.log.print('Number of input reads: {num_read}'.format(num_read=human_and_decoy_filter.I.read_id_list.shape[0]))
    nanopath.log.print('Human read filtering:')
    nanopath.log.print('- number of reads with alignment score >= {threshold} : {num_read}'.format(threshold=human_and_decoy_filter.I.human_min_alignment_score, num_read=num_human_read_with_alignment_score_above_threshold))
    nanopath.log.print('- number of reads with alignment score / read length >= {threshold:.0%} : {num_read}'.format(threshold=human_and_decoy_filter.I.human_min_alignment_score_percent/100, num_read=num_human_read_with_normalized_alignment_score_above_threshold))
    nanopath.log.print('- number of reads satisfying either criteria: {num_read}'.format(num_read=human_and_decoy_filter.O.human_read_id_list.shape[0]))
    nanopath.log.print('Decoy read filtering:')
    nanopath.log.print('- number of reads with alignment score >= {threshold} : {num_read}'.format(threshold=human_and_decoy_filter.I.decoy_min_alignment_score, num_read=num_decoy_read_with_alignment_score_above_threshold))
    nanopath.log.print('- number of reads with alignment score / read length >= {threshold:.0%} : {num_read}'.format(threshold=human_and_decoy_filter.I.decoy_min_alignment_score_percent/100, num_read=num_decoy_read_with_normalized_alignment_score_above_threshold))
    nanopath.log.print('- number of reads satisfying either criteria: {num_read}'.format(num_read=human_and_decoy_filter.O.decoy_read_id_list.shape[0]))
    nanopath.log.print('Number of reads remaining: {num_read}'.format(num_read=human_and_decoy_filter.O.microbe_read_id_list.shape[0]))
    nanopath.log.print('end')


def step_placement_to_species(nanopath, placement_to_species):

    nanopath.log.print('start')
    nanopath.log.print_time()
    nanopath.log.print('Species identification threshold (aligned bp): {min_aligned_bp} '.format(min_aligned_bp=placement_to_species.I.species_ID_min_aligned_bp))

    RAM_dir_name = tempfile.mkdtemp(prefix='placement_to_species.', dir=nanopath.RAM_dir_name)
    ...
    placement_to_species.O.align_list = Align(
                                              ...
                                              aligner_options=shlex.split(nanopath.global_options['alignerThreadOption'] + ' -N 1000 -p 0 -x map-ont'),
                                              paf_path_and_prefix=placement_to_species.I.paf_path_and_prefix,
                                              mapping_only=nanopath.global_options['mapping_only'],
                                              align_all_refseq=True
                                            )
    if nanopath.global_options['debug'] == True:
    ...
    nanopath.log.print('number of alignment: {num_alignment}'.format(num_alignment=placement_to_species.O.best_align_list.shape[0]))
    nanopath.log.print('number of species aligned: {num_species_aligned}'.format(num_species_aligned=placement_to_species.O.aligned_species_list.shape[0]))
    nanopath.log.print('number of species selected: {num_species_selected}'.format(num_species_selected=placement_to_species.O.selected_species_list.shape[0]))
    nanopath.log.print('end')


def step_placement_to_assembly(nanopath, placement_to_assembly):

    nanopath.log.print('start')
    nanopath.log.print_time()

    temp_dir_name = tempfile.mkdtemp(prefix='placement_to_assembly.', dir=nanopath.temp_dir_name)
    RAM_dir_name = tempfile.mkdtemp(prefix='placement_to_assembly.', dir=nanopath.RAM_dir_name)
    ...
    species_target_assembly_list = nanopath.assembly_metadata.get_tax_id(assembly_list=target_assembly_list).merge(
    ...
    split_command = [os.path.join(nanopath.global_options['tool_folder'], 'nanosplit'),]
    ...
    nanopath.log.print('number of additional assembly candidates: {num_candidate}'.format(num_candidate=placement_to_assembly.O.num_assembly_candidate))
    nanopath.log.print('end')


def step_assembly_selection(nanopath, assembly_selection):

    nanopath.log.print('start')
    nanopath.log.print_time()

    nanopath.log.print('Assembly ID will be done on species ID genome set and assembly ID genome set for a species if average depth >= {avg_depth}'.format(avg_depth=assembly_selection.I.assembly_ID_min_average_depth))
    nanopath.log.print('Assembly ID will be done on species ID genome set otherwise')
    ...
    nanopath.log.print('Number of species with assembly selected from species ID genome set and assembly ID genome set: {num_assembly}'.format(num_assembly=species_reached_min_average_depth.shape[0]))
    nanopath.log.print('Number of species with assembly selected from species ID genome set only: {num_species}'.format(num_species=species_not_reached_min_average_depth.shape[0]))
    nanopath.log.print('end')


def step_align_assembly_set(nanopath, align_assembly_set):
...
def step_raw_stat(nanopath, raw_stat):
...
def step_variable_region(nanopath, variable_region):
    ...
    RAM_dir_name = tempfile.mkdtemp(prefix='variable_region.', dir=nanopath.RAM_dir_name)
    ...
    species_id_selected_assembly_id = nanopath.assembly_metadata.get_assembly_path(assembly_list=nanopath.assembly_metadata.get_tax_id(assembly_list=variable_region.I.selected_assembly_list))
    species_id_target_assembly_id = nanopath.assembly_metadata.get_assembly_path(assembly_list=nanopath.assembly_metadata.get_tax_id(assembly_list=variable_region.I.target_assembly_list))
    ...
    for assembly_id_index in range(num_assembly_id):
        ...
        assembly_path = os.path.join(nanopath.global_options['assembly_folder'], species_id_selected_assembly_id.iloc[assembly_id_index]['path'])
        ...
        for query_assembly_id_index in range(num_query_assembly_id):
            ...
            query_assembly_path = os.path.join(nanopath.global_options['assembly_folder'], query_species_id_assembly_id.iloc[query_assembly_id_index]['path'])
            ...
            if os.path.exists(bed_filename) == True:
                ...
                aligner_command = [os.path.join(nanopath.global_options['tool_folder'], nanopath.global_options['aligner']),]
                aligner_command.extend(shlex.split(similarity_option(divergence=variable_region_divergence) + ' -N 1000 -p 0 -c'))
                aligner_command.extend(shlex.split(nanopath.global_options['alignerThreadOption']))
                ...
                aligner_process = subprocess.Popen(aligner_command, close_fds=True, stdout=subprocess.PIPE, stderr=nanopath.aligner_log)
                ...
                try:
                    ...
                    min_alignment_score = nanopath.global_options['min_alignment_score']
    ...
    nanopath.log.print('Variable region summary - average variable span: {average:.2%} - max variable span: {max:.2%}'.format(average=variable_region.O.noise_summary.loc['mean'], max=variable_region.O.noise_summary.loc['max']))
    nanopath.log.print('end')


def step_spike_filter(nanopath, spike_filter):

    nanopath.log.print('start')
    nanopath.log.print_time()

    RAM_dir_name = tempfile.mkdtemp(prefix='spike_filter.', dir=nanopath.RAM_dir_name)
    ...
    nanopath.log.print('filter summary - average noise span: {average:.2%} - max noise span: {max:.2%}'.format(average=spike_filter.O.noise_summary.loc['mean'], max=spike_filter.O.noise_summary.loc['max']))
    nanopath.log.print('end')
...
def step_approx_abundance(nanopath, approx_abundance):

    nanopath.log.print('start')
    nanopath.log.print_time()

    RAM_dir_name = tempfile.mkdtemp(prefix='approx_abundance.', dir=nanopath.RAM_dir_name)
...
def step_microbe_repetitive_region_filter(nanopath, microbe_repetitive_region_filter):
    ...
    assembly_path = nanopath.assembly_metadata.get_assembly_path(assembly_list=abundance_measure)
    assembly_path['compressed_path'] = assembly_path['path'].map(lambda x: os.path.join(nanopath.global_options['assembly_folder'], x))
...
def step_noise_removal(nanopath, noise_removal):

    nanopath.log.print('start')
    nanopath.log.print_time()
    nanopath.log.print('alignments with > {max_noise_overlap:.0%} overlap with noise regions are removed'.format(max_noise_overlap=noise_removal.I.max_align_noise_overlap / 100))

    RAM_dir_name = tempfile.mkdtemp(prefix='noise_removal.', dir=nanopath.RAM_dir_name)
    ...
    nanopath.log.print('{num_removed} out of {num_align} alignments removed'.format(
...
def step_short_alignment_removal(nanopath, short_alignment_removal):

    nanopath.log.print('start')
    nanopath.log.print_time()
    nanopath.log.print('length of alignment >= {threshold}'.format(threshold=short_alignment_removal.I.min_align_length))
    ...
    nanopath.log.print('{num_removed} out of {num_read} reads removed'.format(
...
def step_closing_spike_filter(nanopath, closing_spike_filter):

    nanopath.log.print('start')
    nanopath.log.print_time()

    RAM_dir_name = tempfile.mkdtemp(prefix='closing_spike_filter.', dir=nanopath.RAM_dir_name)
    ...
    nanopath.log.print('filter summary - average noise span: {average:.2%} - max noise span: {max:.2%}'.format(average=closing_spike_filter.O.noise_summary.loc['mean'], max=closing_spike_filter.O.noise_summary.loc['max']))
    nanopath.log.print('{num_removed} out of {num_read} reads removed'.format(
...
def step_combine_with_human_and_decoy(nanopath, combine_with_human_and_decoy):
...
def step_best_alignment(nanopath, best_alignment):
...
def step_separate_human_and_decoy(nanopath, separate_human_and_decoy):
...
def step_unique_alignment(nanopath, unique_alignment):

    nanopath.log.print('start')
    nanopath.log.print_time()
    nanopath.log.print('unique alignment threshold = {threshold:.0%}'.format(threshold=unique_alignment.I.unique_align_threshold / 100))
    ...
    nanopath.log.print('{num_filtered} reads out of {num_total} removed'.format(num_filtered=unique_alignment.O.num_read_before - unique_alignment.O.num_read_after, num_total=unique_alignment.O.num_read_before))
    nanopath.log.print('end')


def step_noise_projection(nanopath, noise_projection):

    nanopath.log.print('start')
    nanopath.log.print_time()
    nanopath.log.print('Number of genus to perform noise projection = {num_genus}'.format(num_genus=noise_projection.I.num_genus))
    nanopath.log.print('Minimum relative abundance to perform noise projection = {relative_abundance:.0%}'.format(relative_abundance=noise_projection.I.min_percent_abundance / 100))
    nanopath.log.print('Read length bin size for simulated reads = {bin_size}'.format(bin_size=noise_projection.I.read_length_bin_size))
    nanopath.log.print('Read length multiplier for maximum read length over average read length = {multiplier}'.format(multiplier=noise_projection.I.read_length_multiplier))
    nanopath.log.print('Error profile for simulated reads = ' + noise_projection.I.error_profile)
    nanopath.log.print('Number of simulated reads to generate = {num_read}'.format(num_read=noise_projection.I.num_read_to_simulate))
    

    RAM_dir_name = tempfile.mkdtemp(prefix='noise_projection.', dir=nanopath.RAM_dir_name)
    ...
    for i in range(num_genus):
        ...
        noise_source_assembly_list = nanopath.assembly_metadata.get_assembly_path(assembly_list=noise_source_assembly_list)
        noise_source_assembly_list['path'] = noise_source_assembly_list['path'].map(lambda x: os.path.join(nanopath.global_options['assembly_folder'], x))
        ...
        for j in range(noise_source_assembly_list['assembly_id'].shape[0]):
            ...
            nanopath.log.print(str(genus_tax_id) + '-' + noise_source_assembly_id + ' : ' + sim_read_filename)

            sim_command = [os.path.join(nanopath.global_options['tool_folder'], nanopath.global_options['read_simulator']), 'linear', '-r', temp_pipe_filename, '-c', os.path.join(nanopath.global_options['tool_folder'], os.path.join(nanopath.global_options['read_simulation_profiles'], noise_projection.I.error_profile)), '-n', str(noise_projection.I.num_read_to_simulate), '--min_len', str(noise_projection.I.min_read_length), '--max_len', str(max_read_length), '-o', sim_read_filename]
            nanopath.log.print(sim_command)
            ...
            nanopath.log.print(zcat_command)
            ...
            sim_align_list = Align(
                                   ...
                                   aligner_options=shlex.split(nanopath.global_options['alignerThreadOption'] + ' -x map-ont -N 1000 -p 0'),
                                   mapping_only=nanopath.global_options['mapping_only'],
                                  )
...
def step_similar_species_marker(nanopath, similar_species_marker):

    nanopath.log.print('start')
    nanopath.log.print_time()
    nanopath.log.print('Number of genus to check for similarity = {num_genus}'.format(num_genus=similar_species_marker.I.num_genus))
    if similar_species_marker.I.alignment_similarity_1 == similar_species_marker.I.alignment_similarity_2:
        nanopath.log.print('A species will be considered to be highly similar if:')
        nanopath.log.print('- Alignment similarity cutoff = {similarity:.0%} and aligned region >= {aligned_region:.0%} of the whole genome'.format(similarity=similar_species_marker.I.alignment_similarity_1 / 100, aligned_region=min(similar_species_marker.I.aligned_region_threshold_1, similar_species_marker.I.aligned_region_threshold_2) / 100))
    else:
        if similar_species_marker.I.similarity_combine_logic == 'and':
            nanopath.log.print('A species will be considered to be highly similar if both of the conditions below are satisfied:')
        else:
            nanopath.log.print('A species will be considered to be highly similar if either or both of the conditions below are satisfied:')
        nanopath.log.print('- Alignment similarity cutoff = {similarity:.0%} and aligned region >= {aligned_region:.0%} of the whole genome'.format(similarity=similar_species_marker.I.alignment_similarity_1 / 100, aligned_region=similar_species_marker.I.aligned_region_threshold_1 / 100))
        nanopath.log.print('- Alignment similarity cutoff = {similarity:.0%} and aligned region >= {aligned_region:.0%} of the whole genome'.format(similarity=similar_species_marker.I.alignment_similarity_2 / 100, aligned_region=similar_species_marker.I.aligned_region_threshold_2 / 100))
    
    RAM_dir_name = tempfile.mkdtemp(prefix='similar_species_marker.', dir=nanopath.RAM_dir_name)
    ...
    for i in range(num_genus):
        ...
        if similar_species_marker.I.alignment_similarity_1 == 80:
            aligner_options_1 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=20)
        elif similar_species_marker.I.alignment_similarity_1 == 90:
            aligner_options_1 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=10)
        elif similar_species_marker.I.alignment_similarity_1 == 95:
            aligner_options_1 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=5)
        elif similar_species_marker.I.alignment_similarity_1 == 98:
            aligner_options_1 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=2)
        elif similar_species_marker.I.alignment_similarity_1 == 99:
            aligner_options_1 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=1)
        else:
            raise RuntimeError('alignment_similarity_1 is invalid')

        if similar_species_marker.I.alignment_similarity_2 == 80:
            aligner_options_2 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=20)
        elif similar_species_marker.I.alignment_similarity_2 == 90:
            aligner_options_2 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=10)
        elif similar_species_marker.I.alignment_similarity_2 == 95:
            aligner_options_2 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=5)
        elif similar_species_marker.I.alignment_similarity_2 == 98:
            aligner_options_2 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=2)
        elif similar_species_marker.I.alignment_similarity_2 == 99:
            aligner_options_2 = nanopath.global_options['alignerThreadOption']  + ' -N 1000 -p 0 '+ similarity_option(divergence=1)
        else:
        ...
        if align_list_1 is not None and align_list_1.empty == False:
            ...
            covered_bp_1 = nanopath.assembly_metadata.get_assembly_length(assembly_list=covered_bp_1)
        ...
        if aligner_options_2 != aligner_options_1:
            ...
            covered_bp_2 = nanopath.assembly_metadata.get_assembly_length(assembly_list=covered_bp_2)
...
def step_noise_detection_statistics(nanopath, noise_detection_statistics):
    ...
    nanopath.log.print('Overall noise summary - average noise span: {average:.2%} - max noise span: {max:.2%}'.format(average=noise_detection_statistics.O.noise_summary.loc['mean'], max=noise_detection_statistics.O.noise_summary.loc['max']))
    nanopath.log.print('end')


def step_noise_removal_statistics(nanopath, noise_removal_statistics):

    nanopath.log.print('start')
    nanopath.log.print_time()

    RAM_dir_name = tempfile.mkdtemp(prefix='noise_removal_statistics.', dir=nanopath.RAM_dir_name)
...
def step_noise_source_statistics(nanopath, noise_source_statistics):
...
def step_max_adjusted_abundance(nanopath, max_adjusted_abundance):
...
def step_read_statistics(nanopath, read_statistics):
...
def step_format_output(nanopath, options):
    ...
    nanopath.assembly_info = get_assembly_info(
                                               assembly_metadata=nanopath.assembly_metadata,
                                               db_conn=nanopath.taxonomy_db_conn,
                                               log=nanopath.log,
                                               assembly_list=pandas.concat([
                                                                            nanopath.human_assembly_list[['assembly_id']],
    ...
    nanopath.assembly_info = nanopath.assembly_info.merge(
                                                          right=nanopath.human_assembly_list.assign(human = lambda x: 1).set_index('assembly_id'),
                                                          how='left', 
                                                          left_on='assembly_id', 
                                                          right_index = True,
                                                          suffixes=['', '_y'],
    ...
    nanopath.assembly_info = nanopath.assembly_info.merge(
                                                          right=nanopath.decoy_assembly_list.assign(decoy = lambda x: 1).set_index('assembly_id'),
                                                          how='left', 
                                                          left_on='assembly_id', 
                                                          right_index = True,
                                                          suffixes=['', '_y'],
    ...
    nanopath.assembly_info = nanopath.assembly_info.merge(
                                                          right=nanopath.species_id_assembly_list.assign(species = lambda x: 1).set_index('assembly_id'),
                                                          how='left', 
                                                          left_on='assembly_id', 
                                                          right_index = True,
                                                          suffixes=['', '_y'],
    ...
    nanopath.assembly_info = nanopath.assembly_info.merge(
                                                          right=nanopath.assembly_id_assembly_list.assign(assembly = lambda x: 1).set_index('assembly_id'),
                                                          how='left', 
                                                          left_on='assembly_id', 
                                                          right_index = True,
                                                          suffixes=['', '_y'],
    ...
    nanopath.assembly_info = nanopath.assembly_info.fillna(0)
    nanopath.assembly_info['human'] = nanopath.assembly_info['human'].astype('int')
    nanopath.assembly_info['decoy'] = nanopath.assembly_info['decoy'].astype('int')
    nanopath.assembly_info['species'] = nanopath.assembly_info['species'].astype('int')
    nanopath.assembly_info['assembly'] = nanopath.assembly_info['assembly'].astype('int')

    if options.output_genome_set == True: #original is options.output_genome_set == False
        nanopath.assembly_info.sort_values(['assembly_id']).to_csv(path_or_buf=file_prefix_with_path + '.genome_set',
    ...
    if options.output_per_read_data == True:
        temp_per_read_dir_name = tempfile.mkdtemp(prefix='per_read.', dir=nanopath.temp_dir_name)
        temp_per_read_prefix_with_path = os.path.join(temp_per_read_dir_name, nanopath.output_prefix)

        nanopath.read_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.read_list', sep='\t', header=True, index=False, columns=consolidated_read_list_col_name)

        nanopath.human_best_align_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.human_list', sep='\t', header=True, index=False, columns=align_list_col_name)
        nanopath.decoy_best_align_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.decoy_list', sep='\t', header=True, index=False, columns=align_list_col_name)
        if options.output_id_signal == True:
            nanopath.id_best_align_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.id_list', sep='\t', header=True, index=False, columns=align_list_col_name)
        if options.output_raw_signal == True:
            nanopath.raw_best_align_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.raw_list', sep='\t', header=True, index=False, columns=align_list_col_name)
        nanopath.align_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.list', sep='\t', header=True, index=False, columns=align_list_col_name)
        nanopath.best_align_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.microbe_list', sep='\t', header=True, index=False, columns=align_list_col_name)
        if options.unique_alignment == True:
            nanopath.unique_align_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.unique_list', sep='\t', header=True, index=False, columns=align_list_col_name)
        if options.output_noise_stat == True:
            nanopath.noise_removal_best_align_list.sort_values(['read_id']).to_csv(path_or_buf=temp_per_read_prefix_with_path + '.noise_list', sep='\t', header=True, index=False, columns=align_list_col_name)
    ...
    if options.output_quality_score_histogram == True:
        temp_quality_score_dir_name = tempfile.mkdtemp(prefix='quality_score.', dir=nanopath.temp_dir_name)
        temp_quality_score_prefix_with_path = os.path.join(temp_quality_score_dir_name, nanopath.output_prefix)

        nanopath.quality_score_bin.to_csv(path_or_buf=temp_quality_score_prefix_with_path + '.quality_score_histogram', sep='\t', header=True, index=False, columns=quality_score_histogram_col_name, float_format='%.2f')
        nanopath.passed_quality_score_bin.to_csv(path_or_buf=temp_quality_score_prefix_with_path + '.passed_quality_score_histogram', sep='\t', header=True, index=False, columns=quality_score_histogram_col_name, float_format='%.2f')
        nanopath.human_quality_score_bin.to_csv(path_or_buf=temp_quality_score_prefix_with_path + '.human_quality_score_histogram', sep='\t', header=True, index=False, columns=quality_score_histogram_col_name, float_format='%.2f')
        nanopath.decoy_quality_score_bin.to_csv(path_or_buf=temp_quality_score_prefix_with_path + '.decoy_quality_score_histogram', sep='\t', header=True, index=False, columns=quality_score_histogram_col_name, float_format='%.2f')
        nanopath.microbe_quality_score_bin.to_csv(path_or_buf=temp_quality_score_prefix_with_path + '.microbe_quality_score_histogram', sep='\t', header=True, index=False, columns=quality_score_histogram_col_name, float_format='%.2f')
        nanopath.aligned_quality_score_bin.to_csv(path_or_buf=temp_quality_score_prefix_with_path + '.aligned_quality_score_histogram', sep='\t', header=True, index=False, columns=quality_score_histogram_col_name, float_format='%.2f')
        nanopath.unaligned_quality_score_bin.to_csv(path_or_buf=temp_quality_score_prefix_with_path + '.unaligned_quality_score_histogram', sep='\t', header=True, index=False, columns=quality_score_histogram_col_name, float_format='%.2f')
    ...
    if options.output_read_length_histogram == True:
        temp_read_length_dir_name = tempfile.mkdtemp(prefix='quality_score.', dir=nanopath.temp_dir_name)
        temp_read_length_prefix_with_path = os.path.join(temp_read_length_dir_name, nanopath.output_prefix)

        nanopath.read_length_bin.to_csv(path_or_buf=temp_read_length_prefix_with_path + '.read_length_histogram', sep='\t', header=True, index=False, columns=read_length_histogram_col_name)
        nanopath.passed_read_length_bin.to_csv(path_or_buf=temp_read_length_prefix_with_path + '.passed_read_length_histogram', sep='\t', header=True, index=False, columns=read_length_histogram_col_name)
        nanopath.human_read_length_bin.to_csv(path_or_buf=temp_read_length_prefix_with_path + '.human_read_length_histogram', sep='\t', header=True, index=False, columns=read_length_histogram_col_name)
        nanopath.decoy_read_length_bin.to_csv(path_or_buf=temp_read_length_prefix_with_path + '.decoy_read_length_histogram', sep='\t', header=True, index=False, columns=read_length_histogram_col_name)
        nanopath.microbe_read_length_bin.to_csv(path_or_buf=temp_read_length_prefix_with_path + '.microbe_read_length_histogram', sep='\t', header=True, index=False, columns=read_length_histogram_col_name)
        nanopath.aligned_read_length_bin.to_csv(path_or_buf=temp_read_length_prefix_with_path + '.aligned_read_length_histogram', sep='\t', header=True, index=False, columns=read_length_histogram_col_name)
        nanopath.unaligned_read_length_bin.to_csv(path_or_buf=temp_read_length_prefix_with_path + '.unaligned_read_length_histogram', sep='\t', header=True, index=False, columns=read_length_histogram_col_name)
    ...
    nanopath.read_stat.to_csv(path_or_buf=file_prefix_with_path + '.read_stat', sep='\t', header=True, index=False, columns=read_stat_col_name)


    # Analysis

    nanopath.human_sequence_list = nanopath.assembly_metadata.get_sequence_tax_id(assembly_list=nanopath.human_assembly_list)
    nanopath.human_sequence_name = get_sequence_name(db_conn=nanopath.taxonomy_db_conn, log=nanopath.log, sequence_list=nanopath.human_sequence_list)
    nanopath.human_best_align_stat = generate_align_stat_group_by_sequence_id(
                                                                              assembly_metadata=nanopath.assembly_metadata,
                                                                              log=nanopath.log,
                                                                              align_list=nanopath.human_best_align_list,
                                                                              sequence_info=nanopath.human_sequence_name,
                                                                             )

    nanopath.decoy_sequence_list = nanopath.assembly_metadata.get_sequence_tax_id(assembly_list=nanopath.decoy_assembly_list)
    nanopath.decoy_sequence_name = get_sequence_name(db_conn=nanopath.taxonomy_db_conn, log=nanopath.log, sequence_list=nanopath.decoy_sequence_list)
    nanopath.decoy_best_align_stat = generate_align_stat_group_by_sequence_id(
                                                                              assembly_metadata=nanopath.assembly_metadata,
                                                                              log=nanopath.log,
                                                                              align_list=nanopath.decoy_best_align_list,
                                                                              sequence_info=nanopath.decoy_sequence_name,
                                                                             )
    
    #test generate_align_stat_by_sequence_id
    #sequence_list = nanopath.assembly_metadata.get_sequence_tax_id(assembly_list=nanopath.assembly_list)
    #sequence_name = get_sequence_name(db_conn=nanopath.taxonomy_db_conn, log=nanopath.log, sequence_list=sequence_list)
    #best_align_stat = generate_align_stat_group_by_sequence_id(
    #                                                                    assembly_metadata=nanopath.assembly_metadata,
    #                                                                    log=nanopath.log,
    #                                                                    align_list=nanopath.best_align_list,
    #                                                                    sequence_info=sequence_name,
    #                                                                    noise_bed=nanopath.noise_bed,
    #                                                                    )
    ...
    if FLAGS.output_id_signal == True:
        nanopath.id_best_align_stat = generate_align_stat_group_by_assembly_id(
                                                                               assembly_metadata=nanopath.assembly_metadata,
                                                                               log=nanopath.log,
                                                                               align_list=nanopath.id_best_align_list,
                                                                               assembly_info=nanopath.assembly_info,
                                                                              )

    nanopath.raw_best_align_stat = generate_align_stat_group_by_assembly_id(
                                                                            assembly_metadata=nanopath.assembly_metadata,
                                                                            log=nanopath.log,
                                                                            align_list=nanopath.raw_best_align_list,
                                                                            assembly_info=nanopath.assembly_info,
                                                                           )

    nanopath.best_align_stat = generate_align_stat_group_by_assembly_id(
                                                                        assembly_metadata=nanopath.assembly_metadata,
                                                                        log=nanopath.log,
                                                                        align_list=nanopath.best_align_list,
                                                                        assembly_info=nanopath.assembly_info,
    ...
    raw_best_align_stat = nanopath.raw_best_align_stat.rename(columns={
    ...
    raw_no_best_align_stat = raw_best_align_stat.merge(
                                                       right=nanopath.best_align_stat[['assembly_id']].assign(joined = lambda x: 1).set_index('assembly_id'),
                                                       how='left', 
                                                       left_on='assembly_id', 
                                                       right_index=True,
                                                       suffixes=['', '_y'],
    ...
    nanopath.best_align_stat = nanopath.best_align_stat.merge(
    ...
    num_assembly_id = nanopath.best_align_stat.shape[0]
    nanopath.best_align_stat = nanopath.best_align_stat.merge(
                                                              right=nanopath.max_adjusted_abundance.set_index('assembly_id'),
                                                              how='inner', 
                                                              left_on='assembly_id', 
                                                              right_index=True,
                                                              suffixes=['', '_y'],
    ...
    if nanopath.best_align_stat.shape[0] != num_assembly_id:
        raise RuntimeError('Assembly ID unmatched')

    nanopath.best_align_stat['adjusted_total_aligned_bp'] = nanopath.best_align_stat[['adjusted_total_aligned_bp', 'max_adjusted_total_aligned_bp']].min(axis=1)
    nanopath.best_align_stat['adjusted_average_depth'] = nanopath.best_align_stat[['adjusted_average_depth', 'max_adjusted_average_depth']].min(axis=1)
    nanopath.best_align_stat['adjusted_covered_percent'] = nanopath.best_align_stat[['adjusted_covered_percent', 'max_adjusted_covered_percent']].min(axis=1)
    nanopath.best_align_stat = nanopath.best_align_stat.drop(['max_adjusted_total_aligned_bp', 'max_adjusted_average_depth', 'max_adjusted_covered_percent'], axis=1)

    nanopath.best_align_stat = pandas.concat([nanopath.best_align_stat, raw_no_best_align_stat], axis=0,sort=True).fillna(0)

    nanopath.best_align_stat = nanopath.best_align_stat.merge(
                                                              right=nanopath.similar_species_marker.set_index('assembly_id'),
                                                              how='left', 
                                                              left_on='assembly_id', 
                                                              right_index=True,
                                                              suffixes=['', '_y'],
    ...
    for key, value in align_stat_col_type_with_pre_noise_with_similar_species_marker_by_assembly_id.items():
        if value == int:
            nanopath.best_align_stat[key] = nanopath.best_align_stat[key].astype(value)


    if options.unique_alignment == True:
        nanopath.unique_align_stat = generate_align_stat_group_by_assembly_id(
                                                                              assembly_metadata=nanopath.assembly_metadata,
                                                                              log=nanopath.log,
                                                                              align_list=nanopath.unique_align_list,
                                                                              assembly_info=nanopath.assembly_info,
                                                                              noise_bed=nanopath.noise_bed,
                                                                             )
        raw_no_unique_align_stat = raw_best_align_stat.merge(
                                                             right=nanopath.unique_align_stat[['assembly_id']].assign(joined = lambda x: 1).set_index('assembly_id'),
                                                             how='left', 
                                                             left_on='assembly_id', 
                                                             right_index=True,
                                                             suffixes=['', '_y'],
        ...
        nanopath.unique_align_stat = nanopath.unique_align_stat.merge(
                                                                      right=raw_best_align_stat[[
                                                                                                 'assembly_id',
                                                                                                 'pre_total_number_of_read',
                                                                                                 'pre_total_read_bp',
        ...
        num_assembly_id = nanopath.unique_align_stat.shape[0]
        nanopath.unique_align_stat = nanopath.unique_align_stat.merge(
                                                                      right=nanopath.max_adjusted_abundance.set_index('assembly_id'),
                                                                      how='inner', 
                                                                      left_on='assembly_id', 
        ...
        if nanopath.unique_align_stat.shape[0] != num_assembly_id:
            raise RuntimeError('Assembly ID unmatched')

        nanopath.unique_align_stat['adjusted_total_aligned_bp'] = nanopath.unique_align_stat[['adjusted_total_aligned_bp', 'max_adjusted_total_aligned_bp']].min(axis=1)
        nanopath.unique_align_stat['adjusted_average_depth'] = nanopath.unique_align_stat[['adjusted_average_depth', 'max_adjusted_average_depth']].min(axis=1)
        nanopath.unique_align_stat['adjusted_covered_percent'] = nanopath.unique_align_stat[['adjusted_covered_percent', 'max_adjusted_covered_percent']].min(axis=1)
        nanopath.unique_align_stat = nanopath.unique_align_stat.drop(['max_adjusted_total_aligned_bp', 'max_adjusted_average_depth', 'max_adjusted_covered_percent'], axis=1)

        nanopath.unique_align_stat = pandas.concat([nanopath.unique_align_stat, raw_no_unique_align_stat], axis=0,sort=True).fillna(0)
        ...
        for key, value in align_stat_col_type_with_pre_noise_with_similar_species_marker_by_assembly_id.items():
            if value == int:
                nanopath.unique_align_stat[key] = nanopath.unique_align_stat[key].astype(value)



    if options.output_human_stat == True:
        nanopath.human_best_align_stat.sort_values(['sequence_length'], ascending=[False]).to_csv(path_or_buf=file_prefix_with_path + '.human_stat', sep='\t', header=True, index=False, columns=align_stat_raw_col_name_by_sequence_id)
    if options.output_decoy_stat == True:
        nanopath.decoy_best_align_stat.sort_values(['total_aligned_bp'], ascending=[False]).to_csv(path_or_buf=file_prefix_with_path + '.decoy_stat', sep='\t', header=True, index=False, columns=align_stat_raw_col_name_by_sequence_id)
    if options.output_id_signal == True:
        nanopath.id_best_align_stat.sort_values(['adjusted_total_aligned_bp'], ascending=[False]).to_csv(path_or_buf=file_prefix_with_path + '.id_stat', sep='\t', header=True, index=False, columns=align_stat_raw_col_name_by_assembly_id)
    if options.output_raw_signal == True:
        nanopath.raw_best_align_stat.sort_values(['adjusted_total_aligned_bp'], ascending=[False]).to_csv(path_or_buf=file_prefix_with_path + '.raw_stat', sep='\t', header=True, index=False, columns=align_stat_raw_col_name_by_assembly_id)
    nanopath.best_align_stat = nanopath.best_align_stat.sort_values(['adjusted_total_aligned_bp', 'pre_total_aligned_bp'], ascending=[False, False])
    nanopath.best_align_stat.to_csv(path_or_buf=file_prefix_with_path + '.microbe_stat', sep='\t', header=True, index=False, columns=align_stat_col_name_with_pre_noise_with_similar_species_marker_by_assembly_id)
    if options.unique_alignment == True:
        nanopath.unique_align_stat.sort_values(['adjusted_total_aligned_bp', 'pre_total_aligned_bp'], ascending=[False, False]).to_csv(path_or_buf=file_prefix_with_path + '.unique_stat', sep='\t', header=True, index=False, columns=align_stat_col_name_with_pre_noise_with_similar_species_marker_by_assembly_id)


    # for downstream processing 

    nanopath.best_align_stat.query('adjusted_total_aligned_bp > 0').sort_values(['adjusted_total_aligned_bp'], ascending=[False]).to_csv(path_or_buf=file_prefix_with_path + '.preport', sep='\t', header=True, index=False, columns=align_stat_col_name_dot_report)

    #testing for sequence id in best_align_stat
    nanopath.sequence_list = nanopath.assembly_metadata.get_sequence_tax_id(assembly_list=nanopath.assembly_list)
    nanopath.sequence_name = get_sequence_name(db_conn=nanopath.taxonomy_db_conn, log=nanopath.log, sequence_list=nanopath.sequence_list)
    nanopath.best_align_stat_by_sequence_id = generate_align_stat_group_by_sequence_id(
                                                                                        assembly_metadata=nanopath.assembly_metadata,
                                                                                        log=nanopath.log,
                                                                                        align_list=nanopath.best_align_list,
                                                                                        sequence_info=nanopath.sequence_name,
                                                                                        )
    nanopath.best_align_stat_by_sequence_id.sort_values(['total_aligned_bp'], ascending=[False]).to_csv(path_or_buf=file_prefix_with_path + '.microbe_stat_by_sequence_id', sep='\t', header=True, index=False, columns=align_stat_raw_col_name_by_sequence_id) 
    nanopath.sequence_info = nanopath.sequence_list[['assembly_id', 'sequence_id', 'sequence_length']].merge(
                                                                                                            right=nanopath.assembly_info.set_index('assembly_id'),
                                                                                                            how='left',
                                                                                                            left_on='assembly_id',
                                                                                                            right_index=True,
                                                                                                            validate='m:1',
    ...
    nanopath.best_align_stat_by_sequence_id_with_assembly_info = nanopath.best_align_stat_by_sequence_id.merge(
                                                                                                        right=nanopath.sequence_info.set_index('sequence_id'),
                                                                                                        how='left',
                                                                                                        left_on='sequence_id',
                                                                                                        right_index = True,
                                                                                                        suffixes=['', '_y'],
    ...
    nanopath.best_align_stat_by_sequence_id_with_assembly_info = nanopath.best_align_stat_by_sequence_id_with_assembly_info.merge(
                                                                                                        right = nanopath.best_align_stat.set_index('assembly_id'),
                                                                                                        how = 'left',
                                                                                                        left_on = 'assembly_id',
                                                                                                        right_index = True,
                                                                                                        suffixes=['', '_y'],
    ...
    #nanopath.best_align_stat_by_sequence_id_with_assembly_info.to_csv(path_or_buf=file_prefix_with_path + '.microbe_stat_with_sequence_id_assembly_info.full', sep='\t', header=True, index=False)
    nanopath.best_align_stat_by_sequence_id_with_assembly_info.to_csv(path_or_buf=file_prefix_with_path + '.microbe_stat_by_sequence_id_assembly_info', sep='\t', header=True, index=False, columns=align_stat_raw_col_name_by_sequence_id_with_assembly_id)
    
	#Independent nanopath report for plasmids
    #nanopath.best_align_stat.to_csv(path_or_buf=file_prefix_with_path + '.micro.info', sep='\t', header=True, index=False)
    #nanopath.decoy_best_align_stat.to_csv(path_or_buf=file_prefix_with_path + '.plasmid.info', sep='\t', header=True, index=False)
    #end of Independent nanopath report for plasmids

    #end of testing

    bam_filter = good_align_list(
                                 align_list=nanopath.align_list,
                                 good_align_threshold=nanopath.global_options['good_alignment_threshold'],
                                )
    ...
    nanopath.best_align_list.sort_values(['read_id', 'sequence_id', 'sequence_from', 'sequence_to']).to_csv(path_or_buf=file_prefix_with_path + '.bam_filter', sep='\t', header=True, index=False, columns=align_list_col_name_bam_filter)
    ...
    if options.output_noise_stat == True:
        temp_noise_analysis_dir_name = tempfile.mkdtemp(prefix='noise_analysis.', dir=nanopath.temp_dir_name)
        temp_noise_analysis_prefix_with_path = os.path.join(temp_noise_analysis_dir_name, nanopath.output_prefix)

        sort_order = nanopath.best_align_stat[['assembly_id']]

        nanopath.noise_stat = nanopath.noise_detection_stat.merge(
                                                                  right=nanopath.noise_removal_stat.set_index('assembly_id'),
                                                                  how='outer', 
                                                                  left_on='assembly_id', 
                                                                  right_index = True,
                                                                  suffixes=['', '_y'],
        ...
        nanopath.noise_stat_with_tax_name = nanopath.noise_stat.merge(
                                                                      right=nanopath.assembly_info.set_index('assembly_id'),
                                                                      how='left', 
                                                                      left_on='assembly_id', 
                                                                      right_index = True,
        ...
        nanopath.noise_stat_with_tax_name = nanopath.assembly_metadata.get_assembly_length(assembly_list=nanopath.noise_stat_with_tax_name, how='left').fillna(0)[noise_stat_col_name_with_tax_name]

        nanopath.noise_stat_with_tax_name = sort_order.merge(
                                                             right=nanopath.noise_stat_with_tax_name.set_index('assembly_id'),
                                                             how='inner', 
                                                             left_on='assembly_id', 
                                                             right_index = True,
                                                             suffixes=['', '_y'],
        ...
        for key, value in noise_stat_col_type_with_tax_name.items():
            if value == int:
                nanopath.noise_stat_with_tax_name[key] = nanopath.noise_stat_with_tax_name[key].astype(value)

        nanopath.noise_stat_with_tax_name.to_csv(path_or_buf=temp_noise_analysis_prefix_with_path + '.noise_stat', sep='\t', header=True, index=False, columns=noise_stat_col_name_with_tax_name)


        source_assembly_info = nanopath.assembly_info[['assembly_id', 'species_tax_id', 'species_tax_name']].rename(columns={'species_tax_id': 'source_species_tax_id', 'species_tax_name': 'source_species_tax_name'})
        target_assembly_info = nanopath.assembly_info[['assembly_id', 'species_tax_id', 'species_tax_name']].rename(columns={'species_tax_id': 'target_species_tax_id', 'species_tax_name': 'target_species_tax_name'})

        nanopath.noise_source_stat_with_tax_name = nanopath.noise_source_stat.merge(
        ...
        nanopath.noise_source_stat_with_tax_name = nanopath.assembly_metadata.get_assembly_length(assembly_list=nanopath.noise_source_stat_with_tax_name.assign(assembly_id = lambda x: x['source_assembly_id']), how='left').fillna(0)
        nanopath.noise_source_stat_with_tax_name = nanopath.noise_source_stat_with_tax_name.rename(columns={'assembly_length': 'source_assembly_length'}).drop(['assembly_id'], axis=1)
        nanopath.noise_source_stat_with_tax_name = nanopath.assembly_metadata.get_assembly_length(assembly_list=nanopath.noise_source_stat_with_tax_name.assign(assembly_id = lambda x: x['target_assembly_id']), how='left').fillna(0)
        nanopath.noise_source_stat_with_tax_name = nanopath.noise_source_stat_with_tax_name.rename(columns={'assembly_length': 'target_assembly_length'}).drop(['assembly_id'], axis=1)[noise_source_stat_col_name_with_tax_name]
        for key, value in noise_source_stat_col_type_with_tax_name.items():
            if value == int:
                nanopath.noise_source_stat_with_tax_name[key] = nanopath.noise_source_stat_with_tax_name[key].astype(value)

        nanopath.noise_source_stat_with_tax_name['source_species_tax_name'] = numpy.where(nanopath.noise_source_stat_with_tax_name['source_assembly_id'] == 'human', 'Human Genome Set', nanopath.noise_source_stat_with_tax_name['source_species_tax_name'])
        nanopath.noise_source_stat_with_tax_name['source_species_tax_name'] = numpy.where(nanopath.noise_source_stat_with_tax_name['source_assembly_id'] == 'decoy', 'Decoy Genome Set', nanopath.noise_source_stat_with_tax_name['source_species_tax_name'])
        nanopath.noise_source_stat_with_tax_name['source_species_tax_name'] = numpy.where(nanopath.noise_source_stat_with_tax_name['source_assembly_id'] == 'unidentified', 'Unidentified', nanopath.noise_source_stat_with_tax_name['source_species_tax_name'])
        nanopath.noise_source_stat_with_tax_name['source_assembly_id'] = numpy.where(nanopath.noise_source_stat_with_tax_name['source_assembly_id'] == 'human', 'N/A', nanopath.noise_source_stat_with_tax_name['source_assembly_id'])
        nanopath.noise_source_stat_with_tax_name['source_assembly_id'] = numpy.where(nanopath.noise_source_stat_with_tax_name['source_assembly_id'] == 'decoy', 'N/A', nanopath.noise_source_stat_with_tax_name['source_assembly_id'])
        nanopath.noise_source_stat_with_tax_name['source_assembly_id'] = numpy.where(nanopath.noise_source_stat_with_tax_name['source_assembly_id'] == 'unidentified', 'N/A', nanopath.noise_source_stat_with_tax_name['source_assembly_id'])

        nanopath.noise_source_stat_with_tax_name = nanopath.noise_source_stat_with_tax_name.sort_values(['target_assembly_id', 'source_assembly_id'])
        nanopath.noise_source_stat_with_tax_name = sort_order.rename(columns={'assembly_id': 'target_assembly_id'}).merge(
                                                                                                                          right=nanopath.noise_source_stat_with_tax_name.set_index('target_assembly_id'),
                                                                                                                          how='inner', 
                                                                                                                          left_on='target_assembly_id', 
                                                                                                                          right_index = True,
                                                                                                                          suffixes=['', '_y'],
        ...
        nanopath.noise_source_stat_with_tax_name.to_csv(path_or_buf=temp_noise_analysis_prefix_with_path + '.noise_source', sep='\t', header=True, index=False, columns=noise_source_stat_col_name_with_tax_name)
        ...
        if options.human_repetitive_region_filter == True or options.microbe_repetitive_region_filter == True:
            ...
            nanopath.similar_noise_span_bp_with_tax_name = nanopath.similar_noise_span_bp.merge(
                                                                                                right=source_assembly_info.set_index('assembly_id'),
                                                                                                how='left', 
                                                                                                left_on='source_assembly_id', 
                                                                                                right_index = True,
            ...
            nanopath.similar_noise_span_bp_with_tax_name = nanopath.assembly_metadata.get_assembly_length(assembly_list=nanopath.similar_noise_span_bp_with_tax_name.assign(assembly_id = lambda x: x['source_assembly_id']), how='left').fillna(0)
            nanopath.similar_noise_span_bp_with_tax_name = nanopath.similar_noise_span_bp_with_tax_name.rename(columns={'assembly_length': 'source_assembly_length'}).drop(['assembly_id'], axis=1)
            nanopath.similar_noise_span_bp_with_tax_name = nanopath.assembly_metadata.get_assembly_length(assembly_list=nanopath.similar_noise_span_bp_with_tax_name.assign(assembly_id = lambda x: x['target_assembly_id']), how='left').fillna(0)
            nanopath.similar_noise_span_bp_with_tax_name = nanopath.similar_noise_span_bp_with_tax_name.rename(columns={'assembly_length': 'target_assembly_length'}).drop(['assembly_id'], axis=1)
            nanopath.similar_noise_span_bp_with_tax_name = nanopath.similar_noise_span_bp_with_tax_name.fillna(0).assign(total_similar_noise_percent = lambda x: x['total_similar_noise_bp'] / x['target_assembly_length'])[similar_noise_span_bp_col_name_with_tax_name]

            for key, value in similar_noise_span_bp_col_type_with_tax_name.items():
                if value == int:
                    nanopath.similar_noise_span_bp_with_tax_name[key] = nanopath.similar_noise_span_bp_with_tax_name[key].astype(value)

            nanopath.similar_noise_span_bp_with_tax_name = nanopath.similar_noise_span_bp_with_tax_name.sort_values(['target_assembly_id', 'source_assembly_id'])
            nanopath.similar_noise_span_bp_with_tax_name = sort_order.rename(columns={'assembly_id': 'target_assembly_id'}).merge(
                                                                                                                                  right=nanopath.similar_noise_span_bp_with_tax_name.set_index('target_assembly_id'),
                                                                                                                                  how='inner', 
                                                                                                                                  left_on='target_assembly_id', 
                                                                                                                                  right_index = True,
                                                                                                                                  suffixes=['', '_y'],
            ...
            nanopath.similar_noise_span_bp_with_tax_name.to_csv(path_or_buf=temp_noise_analysis_prefix_with_path + '.similar_region', sep='\t', header=True, index=False, columns=similar_noise_span_bp_col_name_with_tax_name)
...
def main():
    ...
    nanopath = Placeholder()

    nanopath.global_options = {
    ...
    nanopath.log = Log(os.sys.stderr, 'nanopath')

    nanopath.log.print('program start')
    nanopath.log.print_time()
    ...
    if os.path.exists(os.path.join(nanopath.global_options['config_folder'], FLAGS.human)) == False or os.path.isfile(os.path.join(nanopath.global_options['config_folder'], FLAGS.human)) == False:
        os.sys.exit('Human genome set not found')
    if os.path.exists(os.path.join(nanopath.global_options['config_folder'], FLAGS.decoy)) == False or os.path.isfile(os.path.join(nanopath.global_options['config_folder'], FLAGS.decoy)) == False:
        os.sys.exit('Decoy genome set not found')
    if os.path.exists(os.path.join(nanopath.global_options['config_folder'], FLAGS.species)) == False or os.path.isfile(os.path.join(nanopath.global_options['config_folder'], FLAGS.species)) == False:
        os.sys.exit('Species ID genome set not found')
    if os.path.exists(os.path.join(nanopath.global_options['config_folder'], FLAGS.assembly)) == False or os.path.isfile(os.path.join(nanopath.global_options['config_folder'], FLAGS.assembly)) == False:
        os.sys.exit('Assembly ID genome set not found')


    nanopath.taxonomy_db_conn = sqlite3.connect(nanopath.global_options['taxonomy_db'])

    if FLAGS.temp_folder != '':
        nanopath.temp_dir_name = tempfile.mkdtemp(prefix='nanopath.', dir=FLAGS.temp_folder)
    else:
        nanopath.temp_dir_name = tempfile.mkdtemp(prefix='nanopath.')

    if FLAGS.RAM_folder != '':
        nanopath.RAM_dir_name = tempfile.mkdtemp(prefix='nanopath.', dir=FLAGS.RAM_folder)
    else:
        nanopath.RAM_dir_name = tempfile.mkdtemp(prefix='nanopath.')

    if nanopath.global_options['debug'] == False:
        atexit.register(exit_with_cleanup, temp_root=nanopath.temp_dir_name, RAM_root=nanopath.RAM_dir_name)
    else:
        pybedtools.KEEP_TEMPFILES=True

    pybedtools.helpers.set_bedtools_path(os.path.join(nanopath.global_options['tool_folder'], 'bedtools'))
    pybedtools.helpers.set_tempdir(nanopath.RAM_dir_name)
    if FLAGS.debug == True:
    ...
    nanopath.global_options['max_target_GBase_per_batch'] = ram_available // 1024 // 1024 // 1024 // 8 

    nanopath.global_options['alignerThreadOption'] = '-t ' + str(min(num_core, nanopath.global_options['max_aligner_thread'])) + ' -I ' + str(nanopath.global_options['max_target_GBase_per_batch']) + 'G'
    nanopath.global_options['alignerThreadOption'] = '-t ' + str( nanopath.global_options['max_aligner_thread']) + ' -I ' + str(nanopath.global_options['max_target_GBase_per_batch']) + 'G'
    nanopath.global_options['qcatThreadOption'] = '-t ' + str( nanopath.global_options['max_qcat_thread'])

    nanopath.log.print('Loading assembly metadata')

    nanopath.assembly_metadata = AssemblyMetadata(assembly_folder=nanopath.global_options['assembly_folder'])

    nanopath.log.print('Number of assemblies: {num_assembly}'.format(num_assembly=nanopath.assembly_metadata.get_num_assembly()))
    nanopath.log.print_time()

    if nanopath.global_options['aligner_log'] == '':
        nanopath.aligner_log = os.sys.stderr
    else:
        nanopath.aligner_log = os.open(nanopath.global_options['aligner_log'], flags=os.O_CREAT | os.O_WRONLY, mode=0o644)
    if nanopath.global_options['read_sim_log'] == '':
        nanopath.read_sim_log = os.sys.stderr
    else:
        if FLAGS.noise_projection == True:
            nanopath.read_sim_log = os.open(nanopath.global_options['read_sim_log'], flags=os.O_CREAT | os.O_WRONLY, mode=0o644)

    nanopath.query_filename_list = pandas.DataFrame(FLAGS.query.split(','), columns=['path'])

    nanopath.output_folder = FLAGS.output_folder
    if FLAGS.output_prefix == '':
        temp_query_file_name = os.path.split(nanopath.query_filename_list['path'][0])[1]
        ...
        if last_dot_pos == 0:
            nanopath.output_prefix = temp_query_file_name
        else:
            nanopath.output_prefix = temp_query_file_name[:last_dot_pos]
    else:
        nanopath.output_prefix = FLAGS.output_prefix

    for file in nanopath.query_filename_list['path']:
        if os.path.exists(file) == True:
            nanopath.log.print('input query file: ' + file)
        else:
            nanopath.log.print('input query file: ' + file + ' not found')
            os.sys.exit()
    ...
    adaptor_trimming.I.query_filename_list = nanopath.query_filename_list

    if FLAGS.adaptor_trimming == True:
        step_adaptor_trimming(nanopath, adaptor_trimming)
    ...
    # Output
    # adaptor_trimming.O.query_filename_list    	format: ['path'] (files reside in nanopath.temp_dir_name)
    ...
    read_trimming_and_filter.I.head_crop = nanopath.global_options['head_crop']
    read_trimming_and_filter.I.tail_crop = nanopath.global_options['tail_crop']
    read_trimming_and_filter.I.min_read_length = nanopath.global_options['min_read_length']
    read_trimming_and_filter.I.min_read_quality = nanopath.global_options['min_read_quality']
    ...
    step_read_trimming_and_filter(nanopath, read_trimming_and_filter)
    ...
    # Output
    # read_trimming_and_filter.O.query_filename_list         format: ['path'] (files reside in nanopath.temp_dir_name)
    # read_trimming_and_filter.O.read_info_filename_list     format: ['path'] (files reside in nanopath.temp_dir_name)
    ...
    human_and_decoy_filter.I.human_min_alignment_score = nanopath.global_options['human_filter_alignment_score_threshold']
    human_and_decoy_filter.I.human_min_alignment_score_percent = nanopath.global_options['human_filter_alignment_score_percent_threshold']
    human_and_decoy_filter.I.decoy_min_alignment_score = nanopath.global_options['decoy_filter_alignment_score_threshold']
    human_and_decoy_filter.I.decoy_min_alignment_score_percent = nanopath.global_options['decoy_filter_alignment_score_percent_threshold']

    if FLAGS.human_filter == True and nanopath.global_options['human'] != '':
        nanopath.log.print('Human filter genome set: ' + nanopath.global_options['human'])
        human_and_decoy_filter.I.human_assembly_list = read_genome_set(global_options=nanopath.global_options, genome_set_name=nanopath.global_options['human'])
    else:
        human_and_decoy_filter.I.human_assembly_list = pandas.DataFrame(columns=['assembly_id'])

    if FLAGS.decoy_filter == True and nanopath.global_options['decoy'] != '':
        nanopath.log.print('Decoy filter genome set: ' + nanopath.global_options['decoy'])
        human_and_decoy_filter.I.decoy_assembly_list = read_genome_set(global_options=nanopath.global_options, genome_set_name=nanopath.global_options['decoy'])
    else:
    ...
    if human_and_decoy_filter.I.human_assembly_list.empty == False or human_and_decoy_filter.I.decoy_assembly_list.empty == False:
        step_human_and_decoy_filter(nanopath, human_and_decoy_filter)
    ...
    # Output
    # human_and_decoy_filter.O.query_filename_list      format: ['path'] (files reside in nanopath.temp_dir_name)
    ...
    placement_to_species.I.species_ID_min_aligned_bp = nanopath.global_options['species_id_min_aligned_bp']
    placement_to_species.I.target_assembly_list = read_genome_set(global_options=nanopath.global_options, genome_set_name=nanopath.global_options['species'])

    if FLAGS.output_PAF == True:
        placement_to_species.I.paf_path_and_prefix = os.path.join(nanopath.output_folder, nanopath.output_prefix + '.species')
    else:
        placement_to_species.I.paf_path_and_prefix = None

    nanopath.log.print('Species identification genome set: ' + nanopath.global_options['species'])

    step_placement_to_species(nanopath, placement_to_species)
    ...
    placement_to_assembly.I.target_assembly_list = read_genome_set(global_options=nanopath.global_options, genome_set_name=nanopath.global_options['assembly'])
    placement_to_assembly.I.species_id_assembly_id = placement_to_species.I.target_assembly_list[['assembly_id']]

    nanopath.log.print('Assembly identification genome set: ' + nanopath.global_options['assembly'])

    step_placement_to_assembly(nanopath, placement_to_assembly)
    ...
    assembly_selection.I.assembly_ID_min_average_depth = nanopath.global_options['assembly_id_min_average_depth']
    assembly_selection.I.good_align_threshold = nanopath.global_options['good_alignment_threshold']
    assembly_selection.I.read_id_species_id = placement_to_species.O.read_id_species_id

    step_assembly_selection(nanopath, assembly_selection)
    ...
    if FLAGS.output_PAF == True:
        align_assembly_set.I.paf_path_and_prefix = os.path.join(nanopath.output_folder, nanopath.output_prefix + '.assembly')
    else:
        align_assembly_set.I.paf_path_and_prefix = None

    step_align_assembly_set(nanopath, align_assembly_set)
    ...
    step_raw_stat(nanopath, raw_stat)
    ...
    variable_region.I.variable_region_percent = nanopath.global_options['variable_region_percent']

    if FLAGS.variable_region_adjustment == True:
        step_variable_region(nanopath, variable_region)
        variable_region.O.noise_stat.to_csv(path_or_buf=os.path.join(nanopath.output_folder, nanopath.output_prefix + '.variable_region'), sep='\t', header=True, index=False, columns=['assembly_id', 'variable_span_bp', 'variable_span_percent'])
    else:
    ...
    spike_filter.I.expected_max_depth_stdev = nanopath.global_options['expected_max_depth_stdev']

    if FLAGS.spike_filter == True:
        step_spike_filter(nanopath, spike_filter)
    else:
    ...
    step_noise_removal(nanopath, noise_removal)
    ...
    short_alignment_removal.I.min_align_length = nanopath.global_options['min_alignment_length']

    if FLAGS.short_alignment_filter == True:
        step_short_alignment_removal(nanopath, short_alignment_removal)
    else:
    ...
    closing_spike_filter.I.expected_max_depth_stdev = nanopath.global_options['closing_expected_max_depth_stdev']
    closing_spike_filter.I.max_align_noise_overlap = nanopath.global_options['max_alignment_noise_overlap']

    if FLAGS.closing_spike_filter == True:
        step_closing_spike_filter(nanopath, closing_spike_filter)
    else:
    ...
    step_combine_with_human_and_decoy(nanopath, combine_with_human_and_decoy)
    ...
    step_best_alignment(nanopath, best_alignment)
    ...
    step_separate_human_and_decoy(nanopath, separate_human_and_decoy)
    ...
    unique_alignment.I.unique_align_threshold = nanopath.global_options['unique_alignment_threshold']

    if FLAGS.unique_alignment == True:
        step_unique_alignment(nanopath, unique_alignment)
    else:
    ...
    noise_projection.I.num_genus = nanopath.global_options['number_of_genus_to_perform_noise_projection']
    noise_projection.I.min_percent_abundance = nanopath.global_options['min_percent_abundance_to_perform_noise_projection']
    noise_projection.I.read_length_bin_size = nanopath.global_options['noise_projection_simulated_read_length_bin_size']
    noise_projection.I.read_length_multiplier = nanopath.global_options['noise_projection_simulated_read_length_multiplier']
    noise_projection.I.error_profile = nanopath.global_options['noise_projection_simulated_read_error_profile']
    noise_projection.I.num_read_to_simulate = nanopath.global_options['noise_projection_num_read_to_simulate']
    noise_projection.I.min_read_length = nanopath.global_options['min_read_length']
    noise_projection.I.genus_height = nanopath.global_options['genus_height']

    if FLAGS.noise_projection == True:
        step_noise_projection(nanopath, noise_projection)
    else:
        noise_projection.O.noise_aligned_bp = pandas.DataFrame(columns=noise_projection_aligned_bp_name)
        for key, value in noise_projection_aligned_bp_type.items():
            noise_projection.O.noise_aligned_bp[key] = noise_projection.O.noise_aligned_bp[key].astype(value)
    ...
    similar_species_marker.I.num_genus = nanopath.global_options['similar_species_marker_num_genus']
    similar_species_marker.I.alignment_similarity_1 = nanopath.global_options['similar_species_marker_alignment_similarity_1']
    similar_species_marker.I.aligned_region_threshold_1 = nanopath.global_options['similar_species_marker_aligned_region_threshold_1']
    similar_species_marker.I.alignment_similarity_2 = nanopath.global_options['similar_species_marker_alignment_similarity_2']
    similar_species_marker.I.aligned_region_threshold_2 = nanopath.global_options['similar_species_marker_aligned_region_threshold_2']
    similar_species_marker.I.similarity_combine_logic = nanopath.global_options['similar_species_marker_similarity_combine_logic']
    similar_species_marker.I.genus_height = nanopath.global_options['genus_height']

    if FLAGS.similar_species_marker == True:
        step_similar_species_marker(nanopath, similar_species_marker)
    else:
        similar_species_marker.O.similar_species_marker = pandas.DataFrame(columns=similar_species_marker_name_with_assembly_id)
        for key, value in similar_species_marker_type_with_assembly_id.items():
            similar_species_marker.O.similar_species_marker[key] = similar_species_marker.O.similar_species_marker[key].astype(value)
    ...
    if FLAGS.output_noise_stat == True:
        step_noise_detection_statistics(nanopath, noise_detection_statistics)
    else:
    ...
    if FLAGS.short_alignment_filter == True:
        noise_removal_statistics.I.min_align_length = nanopath.global_options['min_alignment_length']
    else:
        noise_removal_statistics.I.min_align_length = 0

    if FLAGS.output_noise_stat == True:
        step_noise_removal_statistics(nanopath, noise_removal_statistics)
    else:
    ...
    if FLAGS.output_noise_stat == True:
        step_noise_source_statistics(nanopath, noise_source_statistics)
    else:
    ...
    step_max_adjusted_abundance(nanopath, max_adjusted_abundance)
    ...
    step_read_statistics(nanopath, read_statistics)
    ...
    nanopath.assembly_list = assembly_selection.O.assembly_list[['assembly_id']]

    # Genome sets used

    nanopath.human_genome_set = nanopath.global_options['human']
    nanopath.decoy_genome_set = nanopath.global_options['decoy']
    nanopath.species_id_genome_set = nanopath.global_options['species']
    nanopath.assembly_id_genome_set = nanopath.global_options['assembly']

    nanopath.human_assembly_list = human_and_decoy_filter.I.human_assembly_list[['assembly_id']]
    nanopath.decoy_assembly_list = human_and_decoy_filter.I.decoy_assembly_list[['assembly_id']]
    nanopath.species_id_assembly_list = placement_to_species.I.target_assembly_list[['assembly_id']]
    nanopath.assembly_id_assembly_list = placement_to_assembly.I.target_assembly_list[['assembly_id']]

    # Per-read data

    nanopath.read_list = read_statistics.O.read_list
    nanopath.human_best_align_list = separate_human_and_decoy.O.human_best_align_list
    nanopath.decoy_best_align_list = separate_human_and_decoy.O.decoy_best_align_list
    nanopath.align_list = combine_with_human_and_decoy.O.align_list
    nanopath.best_align_list = separate_human_and_decoy.O.best_align_list
    nanopath.unique_align_list = unique_alignment.O.best_align_list
    nanopath.max_adjusted_abundance = max_adjusted_abundance.O.abundance_stat
    nanopath.id_best_align_list = assembly_selection.O.best_align_list
    nanopath.raw_best_align_list = raw_stat.O.best_align_list
    nanopath.noise_removal_best_align_list = noise_removal_statistics.O.noise_best_align_list

    # Similar species marker

    nanopath.similar_species_marker = similar_species_marker.O.similar_species_marker

    # Read statistics

    nanopath.read_stat = read_statistics.O.read_stat
    nanopath.quality_score_bin = read_statistics.O.quality_score_bin
    nanopath.read_length_bin = read_statistics.O.read_length_bin
    nanopath.passed_quality_score_bin = read_statistics.O.passed_quality_score_bin
    nanopath.passed_read_length_bin = read_statistics.O.passed_read_length_bin
    nanopath.human_quality_score_bin = read_statistics.O.human_quality_score_bin
    nanopath.human_read_length_bin = read_statistics.O.human_read_length_bin
    nanopath.decoy_quality_score_bin = read_statistics.O.decoy_quality_score_bin
    nanopath.decoy_read_length_bin = read_statistics.O.decoy_read_length_bin
    nanopath.microbe_quality_score_bin = read_statistics.O.microbe_quality_score_bin
    nanopath.microbe_read_length_bin = read_statistics.O.microbe_read_length_bin
    nanopath.aligned_quality_score_bin = read_statistics.O.aligned_quality_score_bin
    nanopath.aligned_read_length_bin = read_statistics.O.aligned_read_length_bin
    nanopath.unaligned_quality_score_bin = read_statistics.O.unaligned_quality_score_bin
    nanopath.unaligned_read_length_bin = read_statistics.O.unaligned_read_length_bin
    ...
    nanopath.taxonomy_db_conn.close()

    if FLAGS.debug == False:
        try:
            shutil.rmtree(nanopath.temp_dir_name)
        except(FileNotFoundError):
            pass
        try:
            shutil.rmtree(nanopath.RAM_dir_name)
        except(FileNotFoundError):
            pass


    nanopath.log.print('program end')
    nanopath.log.print_time()



if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='nanopath')
    ...
    parser.set_defaults(human_filter=True)
    ...
    parser.set_defaults(similar_species_marker=True)
    ...
    parser.add_argument('--read_simulation_profiles', help='Read simulation profiles', default=cwd+'/tools/nanosim/nanosim_profiles')
    ...
    parser.add_argument('--min_read_length', help='Minimum read length', type=int, default=500)
    ...
    parser.add_argument('--assembly_id_min_average_depth', help='Min average depth to perform assembly selection', type=float, default=0.5)
    ...
    parser.add_argument('--min_alignment_length', help='Minimum alignment length to be considered as evidence', type=int, default=250)
